...
export PYTHONPATH="/path/to/auth-api:${PYTHONPATH}"

### 3. Configure Environment Variables
Create a `.env` file in your auth-api with the necessary configurations. Reference sample.env to see what variables you need to configure
...
2. Configure `PYTHONPATH` to your project's folder location up to `auth-api/src`:
    - set PYTHONPATH=path\to\auth-api\src &&    PYTHONPATH=path\to\auth-api

## Step 2: Start Docker

1. Open a terminal.
2. Navigate to the `auth-api` directory:
    cd auth-api

3. Run the following command to start the services using Docker Compose:
    docker-compose up

## Step 3: Set Up `auth-api`
...
# Github Workflows
you can find a working example here: https://github.com/bcgov/EPIC.auth/tree/main/.github/workflows
